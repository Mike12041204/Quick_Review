...
//todo add_2dir_deg_revise: add two direction deg. All the code using deg need to be revised
...
struct COND_GRAPH
{
	//todo ppadj_lists should have 2 directions.
	int **ppadj_lists;
	int **pplvl2_nbs;
	int num_of_vertices;
	INT_PAGE *pcur_page;
...
struct CLQ_STAT //lower and upper bounds: see Lines 602-607 of graph.cpp
{
	int nmin_cands; //L_X
	int nmax_cands; //U_X
	int nmin_ext_deg; //min_dge based on L_X
};

class Graph
{
// prefix 'm' means it's member of Graph, 'p' is a pointer, 'pp' is a pointer (array) of pointers
	//todo create adj_list for two direction, change lvl2_nbs to 2-dir version
...
//get the int* which record in pape
